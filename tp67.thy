...
(* ATTENTION : variable non définie \<Rightarrow> Undef \<Rightarrow> NonNull
...
(* 🔴 CORRECTION IMPORTANTE :
   après Read, x peut valoir n’importe quoi \<rightarrow> Any, pas NonNull *)
...
(* If : toujours les deux branches + join, comme demandé par le JAR *)
...
(* ====================================================== *)
(* opérateurs abstraits dédiés à san5 (NE PAS TOUCHER SAN4) *)
(* ====================================================== *)
...
(* les autres cas impossible à déterminer *)
"minusAbs5 _ _ = Any"

(* ====================================================== *)
(* Évaluation abstraite san5 *)
(* ====================================================== *)
...
(* ====================================================== *)
(* Évaluation abstraite des conditions pour san5 *)
(* ====================================================== *)
...
(* =========================== *)
(* san6 : analyseur sûr + ok3  *)
(* =========================== *)

(* lookup6 : même chose que lookup,
   on réutilise tabSan et Undef pour "jamais vu" *)

fun eval_const6 :: "tabSan \<Rightarrow> expression \<Rightarrow> int option" where
"eval_const6 tab (Constant c) = Some c" |
"eval_const6 tab (Variable x) =
   (case lookup tab x of
      Undef \<Rightarrow> Some (-1)   
    | _ \<Rightarrow> None)" |
"eval_const6 tab (Sum e1 e2) =
   (case (eval_const6 tab e1, eval_const6 tab e2) of
      (Some v1, Some v2) \<Rightarrow> Some (v1 + v2)
    | _ \<Rightarrow> None)" |
"eval_const6 tab (Sub e1 e2) =
   (case (eval_const6 tab e1, eval_const6 tab e2) of
      (Some v1, Some v2) \<Rightarrow> Some (v1 - v2)
    | _ \<Rightarrow> None)"

(* opérateurs abstraits prudents (pas de règle spéciale NonNull) *)

fun plusAbs6 :: "absInt \<Rightarrow> absInt \<Rightarrow> absInt" where
"plusAbs6 Undef _ = Any" |
"plusAbs6 _ Undef = Any" |
"plusAbs6 Any _ = Any" |
"plusAbs6 _ Any = Any" |
"plusAbs6 Zero a = a" |
"plusAbs6 a Zero = a" |
"plusAbs6 Pos Pos = Pos" |
"plusAbs6 Neg Neg = Neg" |
"plusAbs6 _ _ = Any"

fun minusAbs6 :: "absInt \<Rightarrow> absInt \<Rightarrow> absInt" where
"minusAbs6 Undef _ = Any" |
"minusAbs6 _ Undef = Any" |
"minusAbs6 Any _ = Any" |
"minusAbs6 _ Any = Any" |
"minusAbs6 Zero Pos = Neg" |
"minusAbs6 Zero Neg = Pos" |
"minusAbs6 Pos Neg = Pos" |
"minusAbs6 Neg Pos = Neg" |
"minusAbs6 a Zero = a" |
"minusAbs6 _ _ = Any"

(* évaluation abstraite des expressions pour san6 *)

fun evalAbsExpr6 :: "tabSan \<Rightarrow> expression \<Rightarrow> absInt" where
"evalAbsExpr6 tab (Constant c) =
   (if c < 0 then Neg else if c = 0 then Zero else Pos)" |

(* variable non définie \<Rightarrow> Undef \<Rightarrow> NonNull car evalE renvoie -1 *)
"evalAbsExpr6 tab (Variable x) =
   (case lookup tab x of
      Undef \<Rightarrow> NonNull
    | a \<Rightarrow> a)" |

"evalAbsExpr6 tab (Sum e1 e2) =
   plusAbs6 (evalAbsExpr6 tab e1) (evalAbsExpr6 tab e2)" |

"evalAbsExpr6 tab (Sub e1 e2) =
   minusAbs6 (evalAbsExpr6 tab e1) (evalAbsExpr6 tab e2)"

(* raffinement des IF pour san6 : même idée que san5 *)

fun refine_true6 :: "condition \<Rightarrow> tabSan \<Rightarrow> tabSan" where
"refine_true6 (Eq (Variable x) (Constant n)) tab =
   (if n = 0 then update tab x Zero else tab)" |
"refine_true6 (Eq (Constant n) (Variable x)) tab =
   (if n = 0 then update tab x Zero else tab)" |
"refine_true6 _ tab = tab"

fun refine_false6 :: "condition \<Rightarrow> tabSan \<Rightarrow> tabSan" where
"refine_false6 (Eq (Variable x) (Constant n)) tab =
   (if n = 0 then update tab x NonNull else tab)" |
"refine_false6 (Eq (Constant n) (Variable x)) tab =
   (if n = 0 then update tab x NonNull else tab)" |
"refine_false6 _ tab = tab"

fun evalCondAbs6 :: "tabSan \<Rightarrow> condition \<Rightarrow> bool option" where
"evalCondAbs6 tab (Eq e1 e2) =
   (case (eval_const e1, eval_const e2) of
      (Some c1, Some c2) \<Rightarrow> Some (c1 = c2)
    | _ \<Rightarrow>
       (case (evalAbsExpr6 tab e1, evalAbsExpr6 tab e2) of
          (Zero, Zero) \<Rightarrow> Some True
        | (Zero, NonNull) \<Rightarrow> Some False
        | (Zero, Pos) \<Rightarrow> Some False
        | (Zero, Neg) \<Rightarrow> Some False
...
fun safeExecAbs6 :: "absInt \<Rightarrow> bool" where
"safeExecAbs6 Zero = False" |
"safeExecAbs6 Any = False" |
"safeExecAbs6 Undef = False" |
"safeExecAbs6 Pos = True" |
"safeExecAbs6 Neg = True" |
"safeExecAbs6 NonNull = True"

fun san6_stmt :: "tabSan \<Rightarrow> statement \<Rightarrow> tabSan option" where
"san6_stmt tab Skip = Some tab" |

"san6_stmt tab (Print _) = Some tab" |

"san6_stmt tab (Read x) =
   Some (update tab x Any)" |

"san6_stmt tab (Aff x e) =
   Some (update tab x (evalAbsExpr6 tab e))" |

"san6_stmt tab (Exec e) =
   (case eval_const6 tab e of
...
"san6_stmt tab (Seq s1 s2) =
   (case san6_stmt tab s1 of
      None \<Rightarrow> None
    | Some t1 \<Rightarrow> san6_stmt t1 s2)" |

"san6_stmt tab (If c s1 s2) =
   (let tabT = refine_true6 c tab;
        tabF = refine_false6 c tab
    in case evalCondAbs6 tab c of
         Some True \<Rightarrow> san6_stmt tabT s1
       | Some False \<Rightarrow> san6_stmt tabF s2
       | None \<Rightarrow>
           (case (san6_stmt tabT s1, san6_stmt tabF s2) of
              (Some t1, Some t2) \<Rightarrow> Some (joinTab t1 t2)
            | _ \<Rightarrow> None))"
...
definition san6 :: "statement \<Rightarrow> bool" where
"san6 s \<equiv> (san6_stmt [] s \<noteq> None)"


value "zip [1::nat, 2, 3, 4, 5, 6, 7, 8, 9] (map san6 [bad1, bad2, bad3, bad4, bad5, bad6, bad7, bad8, bad9])"

value "zip [0::nat, 1, 2, 3, 4, 5, 6, 7, 8] (map san6 [ok0, ok1, ok2, ok3, ok4, ok5, ok6, ok7, ok8])"
...
export_code san6 in Scala (case_insensitive)
